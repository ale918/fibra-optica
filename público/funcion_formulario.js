...
const COLOR_BUFFER = { Azul:'#3B82F6',Naranja:'#F97316',Verde:'#22C55E',Café:'#92400E',Gris:'#6B7280',Blanco:'#E5E7EB',Rojo:'#EF4444',Negro:'#111827' };
...
function obtenerGPSCaja() {
  ...
  navigator.geolocation.getCurrentPosition(
    pos => {
      ...
      status.innerHTML = `✅ GPS: <span style="font-family:'IBM Plex Mono',monospace;font-size:11px;">${cajaGpsLat.toFixed(6)}, ${cajaGpsLng.toFixed(6)}</span>`;
      status.style.color = 'var(--accent)';
    },
    () => { status.textContent = '❌ No se pudo obtener GPS'; },
...
function recopilarDatos() {
  ...
  if (actividadesSeleccionadas.has('incidencias')) {
    ...
    if (document.getElementById('inc-cambio_equipos')?.checked) {
      const desc = document.getElementById('inc-ce-desc')?.value.trim();
      incTipos.push({ tipo: 'cambio_equipos', desc });
    ...
    if (document.getElementById('inc-foco_rojo')?.checked) {
      incTipos.push({
        ...
        upc: parseInt(document.getElementById('fr-upc-cant')?.value) || 0,
        apc: parseInt(document.getElementById('fr-apc-cant')?.value) || 0,
      });
    ...
    if (document.getElementById('inc-actualizaciones')?.checked) {
      const desc = document.getElementById('inc-act-desc')?.value.trim();
      incTipos.push({ tipo: 'actualizaciones', desc });
...
async function guardarReporte() {
  ...
  // Si se armó una caja con GPS, registrarla en el mapa
...
function limpiarFormulario() {
  ...
  document.querySelectorAll('.inp-field, .sel-field').forEach(el => {
    if (el.tagName === 'SELECT') el.selectedIndex = 0;
    else el.value = el.type === 'number' && el.id.includes('total') ? '16' : el.type === 'number' ? '' : '';
  });
  document.querySelectorAll('.tag-check input, #rep-ardilla, #rep-arrancada, #rep-atenuacion, #fr-conector-upc, #fr-conector-apc, #fr-fibra').forEach(cb => cb.checked = false);
  ...
  if (gpsStatus) { gpsStatus.textContent = 'Esperando GPS...'; gpsStatus.style.color = 'var(--muted)'; }
}
...
function renderDetallesHistorial(detalles) {
  ...
  if (detalles.fibra) {
    html += `<div style="margin-bottom:8px;font-size:12px;color:var(--muted);">📡 Fibra: <strong style="color:var(--text);">${detalles.fibra.metros}m</strong></div>`;
  }
  if (detalles.cajas) {
    ...
    html += `<div style="margin-bottom:8px;font-size:12px;color:var(--muted);">
      ...
      ${c.buffer?`· Buffer: <strong style="color:var(--text);">${c.buffer}</strong>`:''}
  ...
  if (detalles.mudanza) {
    html += `<div style="margin-bottom:8px;font-size:12px;color:var(--muted);">
      🔄 Mudanza: <strong style="color:var(--text);">${detalles.mudanza.antenas} antenas</strong> · <strong style="color:var(--text);">${detalles.mudanza.routers} routers</strong> retirados
    </div>`;
  }
  if (detalles.odf) {
  ...
  if (detalles.mangas) {
    ...
    html += `<div style="margin-bottom:8px;font-size:12px;color:var(--muted);">
      🔧 Manga: <strong style="color:var(--text);">${m.hilos} hilos</strong> · Sector: ${m.sector||'—'} · Buffer: ${m.buffer||'—'}
    </div>`;
  }
  if (detalles.incidencias?.tipos?.length > 0) {
    ...
    detalles.incidencias.tipos.forEach(t => {
      ...
      html += `<div style="font-size:12px;color:var(--muted);margin-bottom:4px;">
        ...
        ${t.desc?`· <em>${t.desc}</em>`:''}
        ${t.daños?`· ${[t.daños.conectorUpc?'UPC':'',t.daños.conectorApc?'APC':'',t.daños.fibra?'Fibra':''].filter(Boolean).join(', ')}`:''}
  ...
  if (detalles.reparacion) {
    ...
    const daños = [r.ardilla?'Ardilla':'',r.arrancada?'Arrancada':'',r.atenuacion?'Atenuación':''].filter(Boolean).join(', ');
    html += `<div style="margin-bottom:8px;font-size:12px;color:var(--muted);">
      🛠️ Reparación: ${daños||'—'} · Sector: <strong style="color:var(--text);">${r.sector||'—'}</strong>
      · ${r.hilos} hilos · Buffer: ${r.buffer||'—'}
      ${r.up?`· Up: ${r.up}`:''}
    </div>`;
  }
  return html ? `<div style="background:var(--surface);border-radius:8px;padding:10px;margin-bottom:10px;">${html}</div>` : '';
}
...
function exportarReporteExcel(id) {
  ...
  if (!datos.length) datos.push({'Fecha':r.fecha,'Día':dias[d.getDay()],'Actividades':acts,'Integrantes':(r.integrantes||[]).join(', '),'Material':'—','Cantidad':0,'Unidad':'—','Observaciones':r.observaciones||'','IPs':ipsTexto});
...
function obtenerGPS() {
  ...
  if (!navigator.geolocation) {
    if (status) status.innerHTML=`❌ GPS no disponible. <button onclick="activarSeleccionMapa()" style="font-size:11px;padding:2px 8px;border-radius:4px;border:1px solid var(--accent);color:var(--accent);background:none;cursor:pointer;">🗺️ Seleccionar en mapa</button>`;
    return;
  }
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    ()=>{if(status)status.innerHTML=`⚠️ No se pudo obtener GPS. <button onclick="activarSeleccionMapa()" style="font-size:11px;padding:2px 8px;border-radius:4px;border:1px solid var(--accent);color:var(--accent);background:none;cursor:pointer;">🗺️ Seleccionar en mapa</button>`;},
    {enableHighAccuracy:true,timeout:10000}
  );
}
...
function selHilos(id,val='') {
  return `<select id="${id}" class="sel-field" style="flex:1;min-width:90px;"><option value="">Hilo...</option>${HILOS.map(h=>`<option value="${h}" ${val===h?'selected':''}>${h}</option>`).join('')}</select>`;
}

function selBuffers(id,val='') {
  return `<select id="${id}" class="sel-field" style="flex:1;min-width:90px;"><option value="">Buffer...</option>${BUFFERS.map(b=>`<option value="${b}" ${val===b?'selected':''}>${b}</option>`).join('')}</select>`;
}
...
function agregarDistribucion(buf='',hil='',dest='') {
  ...
  div.id=`dist-row-${id}`;
  div.innerHTML=`${selBuffers(`dist-buf-${id}`,buf)}${selHilos(`dist-hil-${id}`,hil)}<input type="text" id="dist-dest-${id}" placeholder="Destino..." value="${dest}" style="flex:2;min-width:120px;background:var(--surface);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:13px;padding:6px 8px;outline:none;" /><button onclick="document.getElementById('dist-row-${id}').remove()" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:16px;">✕</button>`;
  lista.appendChild(div);
}
  ...
  cajasFiltradas.forEach(caja=>{
    ...
    const distHTML=caja.tipo==='principal'&&caja.distribucion?.length>0?`
      <div style="margin-top:8px;border-top:1px solid #e5e7eb;padding-top:6px;">
        ...
        ${caja.distribucion.map(d=>{const dc=COLOR_BUFFER[d.buffer]||'#6b7280';const hc=COLOR_BUFFER[d.hilo]||'#6b7280';return `<div style="font-size:11px;display:flex;align-items:center;gap:4px;margin-bottom:2px;"><span style="width:8px;height:8px;background:${dc};border-radius:50%;display:inline-block;"></span>${d.buffer}<span style="width:8px;height:8px;background:${hc};border-radius:50%;display:inline-block;margin-left:2px;"></span>${d.hilo}${d.destino?`<span style="color:#6b7280;">→ ${d.destino}</span>`:''}</div>`;}).join('')}
      </div>`:'';
      ...
      <div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:10px 12px;margin-bottom:6px;cursor:pointer;" onclick="irACaja(${c.lat},${c.lng})">
        <div style="display:flex;align-items:flex-start;gap:10px;">
          ...
          <div style="flex:1;min-width:0;">
            ...
            <div style="margin-top:5px;"><div style="height:4px;background:var(--surface);border-radius:2px;overflow:hidden;"><div style="width:${pct}%;height:100%;background:${colorBarra};border-radius:2px;"></div></div><div style="font-size:10px;color:var(--muted);margin-top:2px;">${libres} libre${libres!==1?'s':''} de ${c.totalPuertos}</div></div>
          </div>
...
function editarCaja(id) {
  ...
  const distFilas=(c.distribucion||[]).map((d,i)=>{
    ...
    return `<div style="display:flex;gap:8px;align-items:center;margin-bottom:8px;flex-wrap:wrap;" id="${iid}">${selBuffers(`edit-db-${id}-${i}`,d.buffer)}${selHilos(`edit-dh-${id}-${i}`,d.hilo)}<input type="text" id="edit-dd-${id}-${i}" placeholder="Destino..." value="${d.destino||''}" style="flex:2;min-width:120px;background:var(--surface);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:13px;padding:6px 8px;outline:none;" /><button onclick="document.getElementById('${iid}').remove()" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:16px;">✕</button></div>`;
  }).join('');
  const overlay=document.createElement('div');
  overlay.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:500;display:flex;align-items:center;justify-content:center;padding:1rem;';
  overlay.innerHTML=`
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:1.5rem;width:100%;max-width:500px;max-height:90vh;overflow-y:auto;">
      ...
      <div style="display:flex;gap:10px;margin-bottom:10px;">
        <div style="flex:1;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;display:block;margin-bottom:4px;">Tipo</label>
          <select id="edit-caja-tipo" style="background:var(--surface2);border:1px solid var(--border);border-radius:7px;color:var(--text);font-size:13px;padding:8px;outline:none;width:100%;">
        ...
        <div style="flex:2;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;display:block;margin-bottom:4px;">Referencia</label>
          <input type="text" id="edit-caja-ref" value="${c.referencia}" style="background:var(--surface2);border:1px solid var(--border);border-radius:7px;color:var(--text);font-size:13px;padding:8px;outline:none;width:100%;" />
        </div>
      </div>
      <div style="display:flex;gap:10px;margin-bottom:10px;">
        <div style="flex:1;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;display:block;margin-bottom:4px;">Total puertos</label><input type="number" id="edit-caja-total" value="${c.totalPuertos}" style="background:var(--surface2);border:1px solid var(--border);border-radius:7px;color:var(--text);font-size:13px;padding:8px;outline:none;width:100%;" /></div>
        <div style="flex:1;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;display:block;margin-bottom:4px;">Ocupados</label><input type="number" id="edit-caja-ocupados" value="${c.puertosOcupados}" style="background:var(--surface2);border:1px solid var(--border);border-radius:7px;color:var(--text);font-size:13px;padding:8px;outline:none;width:100%;" /></div>
      </div>
      <div style="display:flex;gap:10px;margin-bottom:10px;">
        <div style="flex:1;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;display:block;margin-bottom:4px;">Buffer</label><select id="edit-caja-buffer" class="sel-field"><option value="">Sin buffer</option>${BUFFERS.map(b=>`<option value="${b}" ${c.buffer===b?'selected':''}>${b}</option>`).join('')}</select></div>
        <div style="flex:1;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;display:block;margin-bottom:4px;">Hilo</label><select id="edit-caja-hilo" class="sel-field"><option value="">Sin hilo</option>${HILOS.map(h=>`<option value="${h}" ${c.hilo===h?'selected':''}>${h}</option>`).join('')}</select></div>
      </div>
      ${esPrincipal?`<div style="margin-bottom:10px;"><div style="font-size:11px;color:var(--muted);text-transform:uppercase;margin-bottom:6px;">📡 Hilos distribuidos</div><div id="edit-dist-lista-${id}">${distFilas}</div><button onclick="agregarDistEdit(${id})" style="font-size:12px;padding:4px 10px;margin-top:4px;">+ Agregar hilo</button></div>`:''}
      <div style="display:flex;gap:8px;">
...
function agregarDistEdit(cajaId) {
  ...
  div.innerHTML=`${selBuffers(`edit-db-${cajaId}-${idx}`,'')}${selHilos(`edit-dh-${cajaId}-${idx}`,'')}
    <input type="text" id="edit-dd-${cajaId}-${idx}" placeholder="Destino..." style="flex:2;min-width:120px;background:var(--surface);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:13px;padding:6px 8px;outline:none;" />
...
async function cargarCuadrillaHoy() {
  ...
  if (cuadrilla) {
    ...
    box.innerHTML=`<div style="background:linear-gradient(135deg,rgba(0,212,170,0.1),rgba(0,153,255,0.05));border:1px solid var(--accent);border-radius:12px;padding:1.25rem;margin-bottom:1.25rem;">
    ...
    cuadrillaHoyId=null;
    box.innerHTML=`<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:1rem;margin-bottom:1.25rem;text-align:center;">
    ...
    return `<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:1rem;margin-bottom:8px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
        <div>
          ...
          <div style="font-size:12px;color:var(--muted);margin-top:2px;">📅 ${c.fecha}${reusos.length>0?` <span style="margin-left:8px;font-size:11px;">♻️ ${reusos.length}x</span>`:''}</div>
        </div>
...
function reutilizarCuadrilla(id) {
  ...
  const overlay=document.createElement('div');
  overlay.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:500;display:flex;align-items:center;justify-content:center;padding:1rem;';
  overlay.innerHTML=`<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:1.5rem;width:100%;max-width:420px;">
...
function mostrarModalStock() {
  ...
  cont.innerHTML=MATERIALES_STOCK.map(m=>{const item=stockActual.find(s=>s.material===m.label);return `<div class="mat-row" style="margin-bottom:8px;"><label style="color:var(--text);font-size:13px;text-transform:none;letter-spacing:0;">${m.label}</label><input type="number" min="0" id="stock-${m.id}" value="${item?item.cantidad:0}" style="width:80px;text-align:right;" /><span class="unidad">${m.unidad}</span></div>`;}).join('');
    ...
    return `
      <div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:1rem;margin-bottom:10px;">
        ...
        `:`
          ...
          ${actsHTML}
          ${integrantes.length>0?`<div style="margin-bottom:10px;"><div style="font-size:11px;color:var(--muted);text-transform:uppercase;margin-bottom:3px;">👷 Integrantes</div><div style="font-size:13px;">${integrantes.join(', ')}</div></div>`:''}
          ...
          ${todosMat.length>0?`<div><div style="font-size:11px;color:var(--muted);text-transform:uppercase;margin-bottom:8px;">📦 Materiales</div><table style="width:100%;border-collapse:collapse;"><tbody>${todosMat.map(m=>`<tr><td style="font-size:12px;color:var(--text);padding:5px 8px 5px 0;width:150px;white-space:nowrap;">${m.label}</td><td style="padding:5px 8px;"><div style="height:6px;background:var(--surface);border-radius:3px;overflow:hidden;"><div style="width:100%;height:100%;background:#1D9E75;border-radius:3px;"></div></div></td><td style="font-size:12px;color:var(--muted);padding:5px 0;text-align:right;white-space:nowrap;width:80px;">${m.val} ${m.unidad}</td></tr>`).join('')}</tbody></table></div>`:''}
        `}
      </div>`;
  }).join('');
