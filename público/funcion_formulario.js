    ...
    return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap;">
      <label style="display:flex;align-items:center;gap:6px;cursor:pointer;min-width:110px;">
        ...
        <span style="display:flex;align-items:center;gap:4px;font-size:12px;color:var(--text);">
          <span style="width:8px;height:8px;background:${hc};border-radius:50%;display:inline-block;border:1px solid rgba(255,255,255,0.2);"></span>${h}
        </span>
      </label>
      <input type="text" id="${prefijo}-dest-${grupoId}-${h}" placeholder="Destino..."
  ...
  return `<div id="${prefijo}-grupo-${id}" style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:10px 12px;margin-bottom:8px;">
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;flex-wrap:wrap;">
      ...
      <input type="text" id="${prefijo}-id-${id}" placeholder="ID (Ej: #1, Cable A...)" value="${idPreset}"
        ...
        oninput="actualizarHeaderBuffer('${prefijo}',${id})" />
      <button onclick="document.getElementById('${prefijo}-grupo-${id}').remove()"
  ...
  cont.innerHTML = reportes.map(r => {
    ...
    return `<div class="reporte-card">
      ...
      <div class="reporte-body" id="body-${r.id}" style="display:none;">
        ${cuadrilla?`<div style="background:linear-gradient(135deg,rgba(0,212,170,0.08),rgba(0,153,255,0.03));border:1px solid rgba(0,212,170,0.3);border-radius:10px;padding:1rem;margin-bottom:10px;">
        ...
        ${(r.materiales||[]).length>0?`<table class="mat-table"><thead><tr><th>Material</th><th>Cantidad</th><th>Unidad</th></tr></thead><tbody>${(r.materiales||[]).map(m=>`<tr><td>${m.material}</td><td class="cant">${m.cantidad}</td><td>${m.unidad}</td></tr>`).join('')}</tbody></table>`:''}
        <div style="margin-top:1rem;">
          <div style="font-size:11px;color:var(--muted);text-transform:uppercase;margin-bottom:8px;">📷 Fotos</div>
          <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:10px;">
            ${fotos.map(f=>`<div style="position:relative;"><img src="${f.url}" style="width:90px;height:90px;object-fit:cover;border-radius:8px;border:1px solid var(--border);cursor:pointer;" onclick="verFoto('${f.url}')" /><button onclick="eliminarFoto(${r.id},'${f.public_id}')" style="position:absolute;top:3px;right:3px;background:rgba(0,0,0,0.6);border:none;color:#fff;border-radius:50%;width:20px;height:20px;font-size:11px;cursor:pointer;padding:0;">✕</button></div>`).join('')}
          </div>
...
function renderMarcadores() {
  ...
  cajasAMostrar.forEach(caja => {
    ...
    const popup=`<div style="font-family:'IBM Plex Sans',sans-serif;min-width:200px;">
      ...
      ${caja.buffer||caja.hilo?`<div style="display:flex;gap:6px;margin-bottom:8px;flex-wrap:wrap;">${caja.buffer?`<span style="font-size:11px;background:#f3f4f6;border-radius:4px;padding:2px 6px;display:flex;align-items:center;gap:4px;"><span style="width:8px;height:8px;background:${bufColor};border-radius:50%;display:inline-block;"></span>Buffer: ${caja.buffer}</span>`:''}${caja.hilo?`<span style="font-size:11px;background:#f3f4f6;border-radius:4px;padding:2px 6px;">Hilo: ${caja.hilo}</span>`:''}</div>`:''}
      <div style="margin-bottom:6px;"><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px;"><span>Puertos</span><span>${caja.puertosOcupados}/${caja.totalPuertos}</span></div><div style="height:6px;background:#e5e7eb;border-radius:3px;overflow:hidden;"><div style="width:${pct}%;height:100%;background:${colorBarra};border-radius:3px;"></div></div><div style="font-size:11px;color:#6b7280;margin-top:2px;">${libres} libre${libres!==1?'s':''}</div></div>
      ${distPopup}
            ...
            return `<div style="background:var(--surface2);border:1px solid rgba(226,75,74,0.3);border-radius:8px;padding:10px 12px;margin-bottom:4px;cursor:pointer;" onclick="irACaja(${item.lat},${item.lng})">
              <div style="display:flex;align-items:flex-start;gap:10px;">
                <div style="flex:1;min-width:0;">
                  ...
                  ${item.sector?`<div style="font-size:11px;color:var(--muted);margin-top:2px;">📍 ${item.sector}</div>`:''}
                  <div style="font-size:11px;color:#E24B4A;margin-top:2px;">Fibra ${item.tipoFibra} hilos</div>
                  ${item.distribucion?.length>0?`<div style="font-size:10px;color:var(--muted);margin-top:2px;">📡 ${item.distribucion.length} hilo${item.distribucion.length>1?'s':''} fusionado${item.distribucion.length>1?'s':''}</div>`:''}
                </div>
                ${rolActual==='trabajador'?`<button onclick="event.stopPropagation();eliminarManga(${item.id})" style="font-size:11px;padding:3px 8px;background:none;border:1px solid var(--danger);color:var(--danger);border-radius:4px;cursor:pointer;">✕</button>`:''}
              </div>
            </div>`;
          }
          ...
          return `<div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:10px 12px;margin-bottom:4px;cursor:pointer;" onclick="irACaja(${c.lat},${c.lng})">
            <div style="display:flex;align-items:flex-start;gap:10px;">
              <div style="flex:1;min-width:0;">
                ...
                ${c.buffer||c.hilo?`<div style="display:flex;gap:5px;margin-top:3px;flex-wrap:wrap;">${c.buffer?`<span style="font-size:10px;background:var(--surface);border-radius:3px;padding:1px 5px;display:flex;align-items:center;gap:3px;"><span style="width:6px;height:6px;background:${bufColor};border-radius:50%;display:inline-block;"></span>${c.buffer}</span>`:''}${c.hilo?`<span style="font-size:10px;background:var(--surface);border-radius:3px;padding:1px 5px;">Hilo: ${c.hilo}</span>`:''}</div>`:''}
                ...
                <div style="margin-top:4px;"><div style="height:4px;background:var(--surface);border-radius:2px;overflow:hidden;"><div style="width:${pct}%;height:100%;background:${colorBarra};border-radius:2px;"></div></div><div style="font-size:10px;color:var(--muted);margin-top:2px;">${libres} libre${libres!==1?'s':''} de ${c.totalPuertos}</div></div>
              </div>
              ${rolActual==='trabajador'?`<div style="display:flex;flex-direction:column;gap:4px;"><button onclick="event.stopPropagation();editarCaja(${c.id})" style="font-size:11px;padding:3px 8px;background:var(--surface);border:1px solid var(--border);color:var(--text);border-radius:4px;cursor:pointer;">✏️</button><button onclick="event.stopPropagation();eliminarCaja(${c.id})" style="font-size:11px;padding:3px 8px;background:none;border:1px solid var(--danger);color:var(--danger);border-radius:4px;cursor:pointer;">✕</button></div>`:''}
            </div>
          </div>`;
        }).join('')}
      </div>
...
function editarCaja(id) {
  ...
  const overlay=document.createElement('div');overlay.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:500;display:flex;align-items:center;justify-content:center;padding:1rem;';
  overlay.innerHTML=`<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:1.5rem;width:100%;max-width:500px;max-height:90vh;overflow-y:auto;">
    ...
    <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;">
      <div style="flex:1;min-width:120px;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;display:block;margin-bottom:4px;">Tipo</label><select id="edit-caja-tipo" style="background:var(--surface2);border:1px solid var(--border);border-radius:7px;color:var(--text);font-size:13px;padding:7px;outline:none;width:100%;"><option value="principal" ${c.tipo==='principal'?'selected':''}>📦 Principal</option><option value="cliente" ${c.tipo==='cliente'?'selected':''}>🔌 Cliente</option><option value="pasante" ${c.tipo==='pasante'?'selected':''}>➡️ Pasante</option></select></div>
      <div style="flex:2;min-width:140px;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;display:block;margin-bottom:4px;">Referencia</label><input type="text" id="edit-caja-ref" value="${c.referencia}" style="background:var(--surface2);border:1px solid var(--border);border-radius:7px;color:var(--text);font-size:13px;padding:7px;outline:none;width:100%;" /></div>
    </div>
    <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;">
      <div style="flex:1;min-width:100px;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;display:block;margin-bottom:4px;">Total puertos</label><input type="number" id="edit-caja-total" value="${c.totalPuertos}" style="background:var(--surface2);border:1px solid var(--border);border-radius:7px;color:var(--text);font-size:13px;padding:7px;outline:none;width:100%;" /></div>
      <div style="flex:1;min-width:100px;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;display:block;margin-bottom:4px;">Ocupados</label><input type="number" id="edit-caja-ocupados" value="${c.puertosOcupados}" style="background:var(--surface2);border:1px solid var(--border);border-radius:7px;color:var(--text);font-size:13px;padding:7px;outline:none;width:100%;" /></div>
    </div>
    <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap;">
      <div style="flex:1;min-width:100px;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;display:block;margin-bottom:4px;">Buffer</label><select id="edit-caja-buffer" class="sel-field"><option value="">Sin buffer</option>${BUFFERS.map(b=>`<option value="${b}" ${c.buffer===b?'selected':''}>${b}</option>`).join('')}</select></div>
      <div style="flex:1;min-width:100px;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;display:block;margin-bottom:4px;">Hilo</label><select id="edit-caja-hilo" class="sel-field"><option value="">Sin hilo</option>${HILOS.map(h=>`<option value="${h}" ${c.hilo===h?'selected':''}>${h}</option>`).join('')}</select></div>
    </div>
    ${esPrincipal?`<div style="margin-bottom:12px;"><div style="font-size:11px;color:var(--muted);text-transform:uppercase;margin-bottom:6px;">📡 Hilos distribuidos</div><div id="edit-dist-lista-${id}">${distFilas}</div><button onclick="agregarDistEdit(${id})" style="font-size:12px;padding:4px 10px;margin-top:4px;">+ Agregar hilo</button></div>`:''}
    <div style="display:flex;gap:8px;"><button onclick="guardarEdicionCaja(${id})" style="flex:1;background:var(--accent);color:#000;border:none;border-radius:8px;font-size:13px;font-weight:600;padding:10px;cursor:pointer;">Guardar</button><button onclick="this.closest('div[style*=fixed]').remove()" style="flex:1;background:none;border:1px solid var(--border);border-radius:8px;font-size:13px;color:var(--text);padding:10px;cursor:pointer;">Cancelar</button></div>
  </div>`;
...
// ── Cuadrillas ────────────────────────────────────────────
async function cargarTodasLasCuadrillas(){try{const res=await fetch('/api/cuadrillas');todasLasCuadrillas=await res.json();}catch(e){todasLasCuadrillas=[];}}
async function cargarCuadrillaHoy(){const hoy=fechaLocal();const cuadrilla=todasLasCuadrillas.find(c=>c.fecha===hoy);const box=document.getElementById('cuadrilla-hoy-box');if(!box)return;if(cuadrilla){cuadrillaHoyId=cuadrilla.id;box.innerHTML=`<div style="background:linear-gradient(135deg,rgba(0,212,170,0.1),rgba(0,153,255,0.05));border:1px solid var(--accent);border-radius:12px;padding:1.25rem;margin-bottom:1rem;"><div style="font-size:10px;color:var(--accent);text-transform:uppercase;letter-spacing:0.08em;font-family:'IBM Plex Mono',monospace;margin-bottom:6px;">Tu cuadrilla de hoy</div><div style="font-size:17px;font-weight:600;color:var(--text);margin-bottom:10px;">${cuadrilla.nombre}</div><div style="display:flex;flex-wrap:wrap;gap:6px;">${cuadrilla.integrantes.map(i=>`<span style="font-size:12px;background:rgba(0,212,170,0.15);border:1px solid rgba(0,212,170,0.3);border-radius:20px;padding:4px 12px;color:var(--accent);">👷 ${i}</span>`).join('')}</div></div>`;}else{cuadrillaHoyId=null;box.innerHTML=`<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:1rem;margin-bottom:1rem;text-align:center;"><div style="font-size:13px;color:var(--muted);">No hay cuadrilla asignada para hoy</div><div style="font-size:11px;color:var(--muted);margin-top:3px;">El administrador aún no ha creado la cuadrilla del día</div></div>`;}}
async function cargarCuadrillas(){await cargarTodasLasCuadrillas();const cont=document.getElementById('lista-cuadrillas');if(!cont)return;const originales=todasLasCuadrillas.filter(c=>!c.reutilizada);if(!originales.length){cont.innerHTML='<p class="empty">No hay cuadrillas creadas.</p>';return;}cont.innerHTML=originales.map(c=>{const reusos=todasLasCuadrillas.filter(x=>x.reutilizada&&x.nombre===c.nombre);return `<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:1rem;margin-bottom:8px;"><div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;flex-wrap:wrap;gap:6px;"><div><div style="font-weight:600;color:var(--accent);font-size:14px;">${c.nombre}</div><div style="font-size:12px;color:var(--muted);margin-top:2px;">📅 ${c.fecha}${reusos.length>0?` · ♻️ ${reusos.length}x reutilizada`:''}</div></div><div style="display:flex;gap:6px;"><button onclick="reutilizarCuadrilla(${c.id})" style="font-size:12px;padding:5px 10px;background:var(--surface);border:1px solid var(--accent);color:var(--accent);border-radius:7px;cursor:pointer;">♻️ Reutilizar</button><button class="btn-danger" onclick="eliminarCuadrilla(${c.id})" style="font-size:12px;padding:5px 10px;">Eliminar</button></div></div><div style="display:flex;flex-wrap:wrap;gap:5px;">${c.integrantes.map(i=>`<span style="font-size:12px;background:rgba(0,212,170,0.1);border:1px solid rgba(0,212,170,0.3);border-radius:20px;padding:3px 10px;color:var(--accent);">👷 ${i}</span>`).join('')}</div></div>`;}).join('');}
async function crearCuadrilla(){const nombre=document.getElementById('cuadrilla-nombre').value.trim();const fecha=document.getElementById('cuadrilla-fecha').value;const integrantes=[...document.querySelectorAll('.cuadrilla-int:checked')].map(i=>i.value);if(!nombre){toast('⚠ Ingresa un nombre');return;}if(!fecha){toast('⚠ Selecciona la fecha');return;}if(!integrantes.length){toast('⚠ Selecciona al menos un integrante');return;}try{const res=await fetch('/api/cuadrillas',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nombre,fecha,integrantes,reutilizada:false})});const text=await res.text();let data;try{data=JSON.parse(text);}catch(e){toast('Error del servidor');return;}if(res.ok&&data.ok){toast('✓ Cuadrilla creada');document.getElementById('cuadrilla-nombre').value='';document.querySelectorAll('.cuadrilla-int').forEach(c=>c.checked=false);await cargarCuadrillas();}else{toast('Error: '+(data.error||'Error desconocido'));}}catch(e){toast('Error de conexión');}}
async function eliminarCuadrilla(id){if(!confirm('¿Eliminar esta cuadrilla y sus reusos?'))return;try{const c=todasLasCuadrillas.find(x=>x.id===id);await fetch('/api/cuadrillas/'+id,{method:'DELETE'});if(c){const reusos=todasLasCuadrillas.filter(x=>x.reutilizada&&x.nombre===c.nombre);for(const r of reusos)await fetch('/api/cuadrillas/'+r.id,{method:'DELETE'});}toast('Cuadrilla eliminada');await cargarCuadrillas();}catch(e){toast('Error al eliminar');}}
function reutilizarCuadrilla(id){const c=todasLasCuadrillas.find(x=>x.id===id);if(!c)return;const overlay=document.createElement('div');overlay.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:500;display:flex;align-items:center;justify-content:center;padding:1rem;';overlay.innerHTML=`<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:1.5rem;width:100%;max-width:400px;"><h3 style="color:var(--accent);font-size:13px;margin-bottom:4px;font-family:'IBM Plex Mono',monospace;">♻️ Reutilizar cuadrilla</h3><p style="font-size:13px;color:var(--muted);margin-bottom:1rem;">Mismos integrantes de <strong style="color:var(--text);">${c.nombre}</strong> para nueva fecha.</p><div style="background:var(--surface2);border-radius:8px;padding:10px;margin-bottom:1rem;display:flex;flex-wrap:wrap;gap:5px;">${c.integrantes.map(i=>`<span style="font-size:11px;background:rgba(0,212,170,0.15);border:1px solid rgba(0,212,170,0.3);border-radius:20px;padding:2px 8px;color:var(--accent);">👷 ${i}</span>`).join('')}</div><div style="display:flex;flex-direction:column;gap:4px;margin-bottom:1rem;"><label style="font-size:11px;color:var(--muted);text-transform:uppercase;">Nueva fecha</label><input type="date" id="reutilizar-fecha" style="background:var(--surface2);border:1px solid var(--border);border-radius:7px;color:var(--text);font-size:13px;padding:8px;outline:none;" /></div><div style="display:flex;gap:8px;"><button onclick="confirmarReutilizar(${c.id})" style="flex:1;background:var(--accent);color:#000;border:none;border-radius:8px;font-size:13px;font-weight:600;padding:10px;cursor:pointer;">Crear</button><button onclick="this.closest('div[style*=fixed]').remove()" style="flex:1;background:none;border:1px solid var(--border);border-radius:8px;font-size:13px;color:var(--text);padding:10px;cursor:pointer;">Cancelar</button></div></div>`;const m=new Date();m.setDate(m.getDate()+1);overlay.querySelector('#reutilizar-fecha').value=`${m.getFullYear()}-${String(m.getMonth()+1).padStart(2,'0')}-${String(m.getDate()).padStart(2,'0')}`;document.body.appendChild(overlay);}
async function confirmarReutilizar(id){const c=todasLasCuadrillas.find(x=>x.id===id);if(!c)return;const fecha=document.getElementById('reutilizar-fecha').value;if(!fecha){toast('⚠ Selecciona una fecha');return;}try{const res=await fetch('/api/cuadrillas',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nombre:c.nombre,fecha,integrantes:c.integrantes,reutilizada:true})});const text=await res.text();let data;try{data=JSON.parse(text);}catch(e){toast('Error del servidor');return;}if(res.ok&&data.ok){toast('✓ Cuadrilla reutilizada para '+fecha);document.querySelector('div[style*="position:fixed"]')?.remove();await cargarCuadrillas();}else{toast('Error: '+(data.error||'Error desconocido'));}}catch(e){toast('Error de conexión');}}

// ── Tabs ──────────────────────────────────────────────────
function mostrarTab(tab){['registro','mapa','historial','bodega','indicadores','cuadrillas'].forEach(t=>{const el=document.getElementById('tab-'+t);if(el)el.style.display=t===tab?'block':'none';});document.querySelectorAll('.tab').forEach(btn=>{btn.classList.toggle('active',btn.getAttribute('onclick')?.includes(`'${tab}'`));});if(tab==='mapa')iniciarMapa();if(tab==='historial')cargarHistorial();if(tab==='bodega')cargarBodega();if(tab==='indicadores')iniciarIndicadores();if(tab==='cuadrillas')cargarCuadrillas();}

// ── Conectados ────────────────────────────────────────────
async function cargarConectados(){try{const res=await fetch('/api/conectados');const conectados=await res.json();document.getElementById('conectados-texto').textContent=`🟢 ${conectados.length}`;document.getElementById('conectados-lista').innerHTML=conectados.length===0?'<div style="font-size:12px;color:#6b7280;padding:4px 0;">Nadie conectado</div>':conectados.map(c=>`<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #2a2f3a;"><span style="width:7px;height:7px;background:#00d4aa;border-radius:50%;flex-shrink:0;"></span><div><div style="font-size:13px;color:#e8eaf0;font-weight:500;">${c.nombre}</div><div style="font-size:11px;color:#00d4aa;">En línea</div></div></div>`).join('');}catch(e){}}
function toggleConectados(){const popup=document.getElementById('conectados-popup');popup.style.display=popup.style.display==='none'?'block':'none';if(popup.style.display==='block')cargarConectados();}
document.addEventListener('click',e=>{const badge=document.getElementById('conectados-badge');const popup=document.getElementById('conectados-popup');if(popup&&badge&&!badge.contains(e.target)&&!popup.contains(e.target))popup.style.display='none';});
function toast(msg){const t=document.getElementById('toast');t.textContent=msg;t.classList.add('show');setTimeout(()=>t.classList.remove('show'),2500);}
async function cerrarSesion(){await fetch('/api/logout',{method:'POST'});window.location.href='/';}

// ── Bodega ────────────────────────────────────────────────
async function cargarBodega(){try{const res=await fetch('/api/bodega');stockActual=await res.json();renderStock();setTimeout(()=>llenarSelectMaterial(),100);cargarMovimientos();}catch(e){document.getElementById('tabla-stock').innerHTML='<p class="empty">Error.</p>';}}
function renderStock(){const cont=document.getElementById('tabla-stock');if(!stockActual.length){cont.innerHTML='<p class="empty">No hay stock registrado.</p>';return;}cont.innerHTML=`<table class="mat-table"><thead><tr><th>Material</th><th>Stock</th><th>Estado</th></tr></thead><tbody>${stockActual.map(s=>`<tr><td>${s.material}</td><td class="cant">${s.cantidad}</td><td><span style="color:${s.cantidad>5?'var(--accent)':s.cantidad>0?'#f59e0b':'var(--danger)'};font-size:12px;">${s.cantidad>5?'✓ Disponible':s.cantidad>0?'⚠ Poco':'✕ Sin stock'}</span></td></tr>`).join('')}</tbody></table>`;}
function llenarSelectMaterial(){const sel=document.getElementById('mov-material');if(!sel)return;sel.innerHTML=MATERIALES_STOCK.map(m=>`<option value="${m.label}">${m.label}</option>`).join('');}
function mostrarModalStock(){const cont=document.getElementById('modal-stock-items');cont.innerHTML=MATERIALES_STOCK.map(m=>{const item=stockActual.find(s=>s.material===m.label);return `<div class="mat-row" style="margin-bottom:8px;"><label style="color:var(--text);font-size:12px;text-transform:none;letter-spacing:0;">${m.label}</label><input type="number" min="0" id="stock-${m.id}" value="${item?item.cantidad:0}" style="width:70px;text-align:right;" /><span class="unidad">${m.unidad}</span></div>`;}).join('');document.getElementById('modal-stock').style.display='flex';}
function cerrarModalStock(){document.getElementById('modal-stock').style.display='none';}
async function guardarStock(){try{for(const m of MATERIALES_STOCK){const el=document.getElementById('stock-'+m.id);const cantidad=el?parseFloat(el.value)||0:0;const res=await fetch('/api/bodega/stock',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({material:m.label,cantidad})});const data=await res.json();if(!data.ok){toast('Error: '+data.error);return;}}cerrarModalStock();await cargarBodega();toast('✓ Stock actualizado');}catch(e){toast('Error: '+e.message);}}
async function registrarMovimiento(){const tipo=document.getElementById('mov-tipo').value;const material=document.getElementById('mov-material').value;const cantidad=parseFloat(document.getElementById('mov-cantidad').value);const responsable=document.getElementById('mov-responsable').value.trim();const nota=document.getElementById('mov-nota').value.trim();if(!cantidad||cantidad<=0){toast('⚠ Ingresa una cantidad válida');return;}if(!responsable){toast('⚠ Ingresa el responsable');return;}try{const res=await fetch('/api/bodega/movimiento',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tipo,material,cantidad,responsable,nota})});const data=await res.json();if(data.ok){toast('✓ Movimiento registrado');document.getElementById('mov-cantidad').value='';document.getElementById('mov-responsable').value='';document.getElementById('mov-nota').value='';await cargarBodega();}else{toast('Error: '+data.error);}}catch(e){toast('Error de conexión');}}
async function cargarMovimientos(){try{const res=await fetch('/api/bodega/movimientos');const movimientos=await res.json();const cont=document.getElementById('tabla-movimientos');if(!movimientos.length){cont.innerHTML='<p class="empty">No hay movimientos.</p>';return;}const colores={salida:'var(--danger)',entrada:'var(--accent)',devolucion:'#f59e0b'};cont.innerHTML=`<table class="mat-table"><thead><tr><th>Fecha</th><th>Tipo</th><th>Material</th><th>Cant.</th><th>Responsable</th><th>Nota</th><th></th></tr></thead><tbody>${movimientos.map(m=>`<tr><td style="font-size:11px;color:var(--muted);">${m.fecha}</td><td><span style="color:${colores[m.tipo]};font-weight:600;font-size:12px;text-transform:capitalize;">${m.tipo}</span></td><td style="font-size:12px;">${m.material}</td><td class="cant">${m.cantidad}</td><td style="font-size:12px;">${m.responsable}</td><td style="color:var(--muted);font-size:12px;">${m.nota||'—'}</td><td><button class="del-btn" onclick="eliminarMovimiento(${m.id})">✕</button></td></tr>`).join('')}</tbody></table>`;}catch(e){}}
async function eliminarMovimiento(id){if(!confirm('¿Eliminar este movimiento?'))return;try{const res=await fetch('/api/bodega/movimientos/'+id,{method:'DELETE'});const data=await res.json();if(data.ok){toast('Movimiento eliminado');cargarMovimientos();}}catch(e){toast('Error al eliminar');}}
function exportarBodegaExcel(){if(!stockActual.length){toast('⚠ No hay stock');return;}const datos=stockActual.map(s=>({'Material':s.material,'Stock disponible':s.cantidad}));const ws=XLSX.utils.json_to_sheet(datos);const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'Stock');XLSX.writeFile(wb,`bodega_${new Date().toISOString().slice(0,10)}.xlsx`);toast('✓ Stock exportado');}
